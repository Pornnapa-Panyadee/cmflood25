...
export default function CnxTif() {
  ...
  useEffect(() => {
    async function initMap() {
      ...
      const map = L.map("map", { center: [18.745, 98.99], zoom: 12 })
      mapRef.current = map
      ...
      const loadKml = async (url: string, color: string, weight = 1) => {
        ...
        const geo = kml(xml)
        return L.geoJSON(geo, { style: { color, weight, opacity: 0.5, fillOpacity: 0.1 } })
      }

      try {
        // pingRiver = await loadKml("/data/KML/stream.kml", "#529decff", 1.5)
        pingRiver = await loadKml("/data/KML/river_main.kml", "#A8D8FF", 2)
        //pingRiver_New = await loadKml("/data/KML/cnx_stream.kml", "#529decff", 1.5)
        roadLayer = await loadKml("/data/KML/cnx_road_clip1.kml", "rgba(83, 20, 11, 0.52)", 1)
        //buildingLayer = await loadKml("/data/KML/river_main.kml", "#ffe495ff", 1)
        //buildingLayer = await loadKml("/data/KML/cnx_building_clip1.kml", "#ffe495ff", 1)
        //buildingLayer = await loadKml("/data/KML/cnx_building_focus.kml", "rgba(243, 218, 252, 0.83)", 1)
        pingRiver.addTo(map)
      ...
      try {
        const poleRes = await fetch("/data/New_floodMark_2025_fixed_like_pole.geojson")
