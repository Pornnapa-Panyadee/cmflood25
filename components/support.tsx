...
export default function CnxTif() {
  ...
  useEffect(() => {
    ...
    async function initMap() {
      ...
      const map = L.map(mapElRef.current, { center: [18.787563, 99.003968], zoom: 10 })
      mapRef.current = map
      ...
      const loadPumpPointsLayer = async (url: string) => {
        ...
        const layer = L.geoJSON(geojson, {
          ...
          onEachFeature: (feature, lyr) => {
            ...
            const location = props.location ?? props.Location ?? props["ที่อยู่"] ?? "-"
            const area = props.area ?? props.Area ?? props["พื้นที่"] ?? "-"
            const machine = props.machine_count ?? props["ขนาด"] ?? props.count ?? props.Count ?? null
            const countM = props.machine_count ?? props["จำนวน"] ?? props.count ?? props.Count ?? null
            const capacity = props.capacity ?? props["อัตราสูบ"] ?? null
            ...
            const headerHtml = `<div class="popup-header pump"> <b>💧 จุดเครื่องสูบน้ำ </b><hr></div>`

            const popupHtml = `
              <div class="popup-wrap">
                ${headerHtml}
                <div class="popup-row"><b>รหัส: </b>${escapeHtml(String(pid))}</div>
                ...
                ${machine ? `<div class="popup-row"><b>-ขนาด:</b> ${escapeHtml(machine)} ${escapeHtml(countM)}</div>` : ""}
      ...
      setTimeout(() => {
        map.invalidateSize(true)
        map.setView([18.787563, 99.003968], 12)
        //if (bounds.isValid()) {
  ...
  return <div ref={mapElRef} className="w-full h-[80vh]" />
}
